...
WARNING_VIBRATION_ON = 0.3
WARNING_VIBRATION_OFF = 0.3
WARNING_VIBRATION_COUNT = 2
...
for output_pin in (MOTOR_PIN, LED_R, LED_G, LED_B):
...
def main():
    # systemctl stop sends SIGTERM. Handle it so that camera, motor, LED and
            ...
            if normalized_delta is not None:
                ...
                if bad_posture:
                    ...
                    if bad_timer < BAD_POSTURE_LIMIT:
                        ...
                        # Vibrate twice without blocking camera processing:
                        # 0.3 s ON -> 0.3 s OFF -> 0.3 s ON -> OFF.
                        if warning_vibration_start is None:
                            warning_vibration_start = current_time

                        pulse_cycle = (
                            WARNING_VIBRATION_ON
                            + WARNING_VIBRATION_OFF
                        )
                        pulse_elapsed = (
                            current_time - warning_vibration_start
                        )
                        sequence_duration = (
                            pulse_cycle * WARNING_VIBRATION_COUNT
                        )

                        if (
                            pulse_elapsed < sequence_duration
                            and pulse_elapsed % pulse_cycle
                            < WARNING_VIBRATION_ON
                        ):
                            motor_on()
                        else:
                        ...
                        warning_vibration_start = None
                        # Keep vibrating continuously while posture is BAD.
                        motor_on()
                else:
                    bad_timer = 0.0
                    warning_vibration_start = None
