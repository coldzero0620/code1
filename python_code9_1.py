#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Version 12 FULL-FOV FINAL (charging UI + BLE-loss white blink).
#
...
#   - BLE N/W/B/P/S, heartbeat, fast reconnect/resync and WARNING reminder
...
#   - key hints at top-left; live status/score table at bottom-left
#   - SIDE and MODE stay internal and are hidden from the live UI
#   - posture detection, LED and BLE continue after the UI is closed
#   - wristband battery/charging telemetry is received by BLE Notify and shown in UI
#   - BLE loss indicator: WHITE blinks every 0.5 seconds
...
LED_BLE_LOST_BLINK_SECONDS = 0.5
...
class LedStatusWorker:
    """Shows posture color, or a blinking white LED without BLE."""
        ...
        while not self._stop_event.is_set():
            ...
            if self._runtime_state.is_ble_connected():
                ...
                blink_step = int(
                    ...
                    / LED_BLE_LOST_BLINK_SECONDS
                )
                target_color = "WHITE" if blink_step % 2 == 0 else "OFF"
...
BLE_SCAN_TIMEOUT_SECONDS = 2.0
BLE_RETRY_SECONDS = 0.25
...
class BlePostureBand:
    ...
    def _handle_battery_notification(self, sender, data):
        try:
            ...
            charging = None
            if len(parts) == 4:
                charging_token = parts[3].strip().upper()
                if charging_token in {
                    "1", "TRUE", "YES", "ON", "CHARGING"
                }:
                    charging = True
                elif charging_token in {
                    "0", "FALSE", "NO", "OFF", "NOT_CHARGING",
                    "IDLE", "FULL",
                }:
                    charging = False
                else:
                    raise ValueError(
                        f"unknown charging value: {parts[3]!r}"
                    )
            ...
            charging_text = (
                "YES" if charging is True
                else "NO" if charging is False
                else "--"
            )
            print(
                f"[BLE] Battery: {percent}% ({voltage:.2f}V), "
    ...
    def _matches_target(self, device, advertisement_data):
        # After the first connection, the stable ESP address can be matched
        # from the first advertisement without waiting for a scan response.
        if (
        ...
        if BLE_SERVICE_UUID.casefold() in advertised_uuids:
            return True
    ...
    async def _find_device(self):
        # Service UUID matching can succeed on the primary advertisement;
        # name-only matching may have to wait for a separate scan response.
...
def draw_monitor_overlay(frame, snapshot):
    ...
    if charging is True:
        charging_text = "CHARGING: YES"
    elif charging is False:
        charging_text = "CHARGING: NO"
    else:
    ...
    for row_index, (text_value, color, scale, thickness) in enumerate(rows):
        y = first_y + row_index * row_gap
        cv2.putText(
            frame,
            text_value,
            (25, y),
